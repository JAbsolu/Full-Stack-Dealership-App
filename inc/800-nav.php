<nav class="navbar navbar-expand-sm bg-light">
  <div class="container-fluid w-100"> 
    <a class="navbar-brand text-blue fs-5" href="index.php"><b>JVW</b></a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#collapsibleNavbar">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="collapsibleNavbar">
      <ul class="navbar-nav text-dark">
        <li class="nav-item">
          <a class="nav-link " aria-current="page" href="index.php">Home</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="inventory.php">Our inventory</a>
        </li>
        <li class="nav-item">
         <a class="nav-link" href="numbers-input.php">Calculate a payment</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="contact.php">Contact us</a>
        </li>
        <!-- <li class="nav-item dropdown text-dark">
          <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" id="admin">Tables</a>
          <ul class="dropdown-menu">
            <li><a class="dropdown-item" href="fp99-build-project-tables.php">Build Tables</a></li>
            <li><a class="dropdown-item" href="fp99-view-project-tables.php">View Tables</a></li>
          </ul>
        </li> -->
      </ul>
    </div>
  </div>
</nav>
<script src="js/Authorization.js"></script>